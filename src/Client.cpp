...
void Client::listener(HTTP *http_class)
{
	...
	while (http_class->activeListen)
	{
		...
		{
			perror(e.what());
			std::cerr << "Ignoring petition" << std::endl;
		...
		std::cout << "\nClient: Sending data to server..." << std::endl;
		(void)write(this->network_socket,
					request.buffer.c_str(),
					request.buffer_size);
		std::cout << "Client: End of transaction (EOT)" << std::endl;
		...
		std::cout << "Client: Receiving data from server" << std::endl;
		...
		do
		{
			...
			if (!found_size)
			{
				...
				{
					std::cout << "'Content-Lenght' header not found" << std::endl;
					std::cout << "Max bytes: "
		...
		std::cout << "Client: End of transaction (EOT)\n"
				  << std::endl;
